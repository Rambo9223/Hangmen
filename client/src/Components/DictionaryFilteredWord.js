...
function DictionaryFileredWord(props) {
  ...
  let indexes = props.indexes; //array of indexes passed from GetWordIndexes

  let toArray = Object.keys(dictionary); //the words from the dictionary object

  console.log(toArray.filter((word) => word.length<=5));
...
let Arrayfiltered = toArray.filter(
  ...
  let wordIndex = indexes[id]; //the index number at id
